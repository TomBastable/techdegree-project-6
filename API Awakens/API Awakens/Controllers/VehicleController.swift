...
class VehicleController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        client.getVehicles { (vehicles, error) in
            ...
            //assign vehicles, reload picker, setup view and do relevant loading setup
    ...
    @IBAction func conversionRate(_ sender: UIButton) {
        ...
        if conversionRate == 0{
            ...
            alert.addAction(UIAlertAction(title: "Save", style: .default, handler: { [weak alert] (_) in
                ...
                if conversionInt == 0 || conversionInt < 0{
                    self.displayAlertWith(error: SWAPIError.invalidConversionRate)
                    return
                }
    ...
    func displayAlertWith(error: Error){
        ...
        case SWAPIError.invalidConversionRate:
            ...
            subTitle = "A conversion rate may only be made of numbers and cannot be 0 (Zero) or a negative number. Conversion rate not set. Please try again."
            buttonTitle = "Ok"
